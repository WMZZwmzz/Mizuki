...
export const siteConfig: SiteConfig = {
	title: keystaticSettings.title || "Mizuki",
	subtitle: keystaticSettings.subtitle || "One demo website",
	siteURL: keystaticSettings.siteURL || "https://mizuki.mysqil.com/", // 请替换为你的站点URL，以斜杠结尾
	siteStartDate: "2026-07-05", // 站点开始运行日期，用于站点统计组件计算运行天数

	lang: SITE_LANG as SiteConfig["lang"],

	themeColor: {
		hue: keystaticSettings.themeHue ?? 240, // 主题色的默认色相，范围从 0 到 360
		fixed: keystaticSettings.themeFixed ?? false, // 对访问者隐藏主题色选择器
	},

	// 特色页面开关配置（Keystatic 设置优先，否则使用默认值）
	featurePages: {
		anime: keystaticSettings.pageAnime ?? true,
		diary: keystaticSettings.pageDiary ?? true,
		friends: keystaticSettings.pageFriends ?? false,
		projects: keystaticSettings.pageProjects ?? true,
		skills: keystaticSettings.pageSkills ?? true,
		timeline: keystaticSettings.pageTimeline ?? true,
		albums: keystaticSettings.pageAlbums ?? true,
		devices: keystaticSettings.pageDevices ?? false,
	},
	...
	navbarTitle: {
		...
		mode: "text-icon",
		// 顶栏标题文本
		text: keystaticSettings.navbarTitleText || "MizukiUI",
		// 顶栏标题图标路径，默认使用 public/assets/home/home.webp
		icon: keystaticSettings.navbarTitleIcon || "assets/home/home.webp",
		// 网站Logo图片路径
	...
	bilibili: {
		vmid: "1986470618", // 在此处设置你的Bilibili用户ID (uid)，例如 "1129280784"
	...
	diaryApiUrl: keystaticSettings.diaryApiUrl || "",
	...
	// 壁纸模式配置
	wallpaperMode: {
		// 默认壁纸模式：banner=顶部横幅，fullscreen=全屏壁纸，overlay=覆盖背景，none=无壁纸
		// 首页默认全屏 hero，滚动时由 HeroOverlayScroll 平滑过渡到 overlay 视觉
	...
	banner: {
		...
		carousel: {
			enable: keystaticHomepage.carouselEnable ?? true,
			interval: keystaticHomepage.carouselInterval ?? 3,
			switchable: true,
		},

		waves: {
			enable: keystaticHomepage.wavesEnable ?? true,
		...
		homeText: {
			enable: true,
			title: keystaticHomepage.homeTitle || "わたしの部屋",
			switchable: true,
